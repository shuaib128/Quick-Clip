...
from Utils.database import (
    get_all_video_data, get_video_data_by_id,
    delete_video_by_id
)
from Utils.increase_playback_speed import start_increase_timestamps
...
class ThumbnailWorker(QThread):
    ...
    def run(self):
        ...
        video_data_list = get_all_video_data()
...
class CustomListItem(QWidget):
    ...
    def __init__(self, video_id, icon, file_name):
        super().__init__()
        self.video_path = f"Videos\\{file_name}"
        
        # Create the layout
        layout = QHBoxLayout(self)
        layout.addStretch(1)
        
        # Quick clip the button
        self.quick_click_btn = QPushButton("Quick Clip", self)
        self.quick_click_btn.setFixedSize(100, 25)
        self.quick_click_btn.clicked.connect(
            lambda: self.on_btn_clicked(video_id, file_name)
        )
        layout.addWidget(self.quick_click_btn)
    ...
    # Quick clip the video.
    def on_btn_clicked(self, video_id, file_name):
        data = get_video_data_by_id(video_id)

        # Create a new thread for the start_increase_timestamps function
        timestamSpeedThread = threading.Thread(
            target=start_increase_timestamps, 
            args=(file_name, self.video_path, data["intervals"])
        )
        
        # Start the thread
        timestamSpeedThread.start()
...
class VideoPage(QWidget):
    def __init__(self, parent=None):
        ...
        self.folder_path = "Videos"
        layout = QVBoxLayout(self)
        layout.setContentsMargins(25, 30, 25, 0)

        label = QLabel("Select the video which you want to quick clip.")
